...
function ThoughtsView({ items, createItem, updateItem, uploadFile, notify, selectedItem, openItem }: WorkspaceActions & { notify:(message:string)=>void; selectedItem?: WorkspaceItem; openItem:(item:WorkspaceItem)=>void }) {
  ...
  const [quick,setQuick]=useState(""); const [mobileEditor,setMobileEditor]=useState(Boolean(selectedItem));
  const [selection,setSelection]=useState<{text:string;start:number;end:number;x:number;y:number}|null>(null);
  const [taskDialog,setTaskDialog]=useState(false); const titleRef=useRef<HTMLInputElement>(null); const searchRef=useRef<HTMLInputElement>(null);

  useEffect(()=>{if(!active)return;queueMicrotask(()=>{setTitle(active.title==="无标题记录"?"":active.title);setContent(text(active,"content"));setNoteType(text(active,"type","quick") as NoteType);setStatus(text(active,"status","inbox") as NoteStatus);setTags(list(active,"tags"));setSaving("saved");});},[active?.id]);
  ...
  async function newNote(type:NoteType,initial=""){const body=initial||noteTemplates[type];const item=await createItem({kind:"thought",title:body.split(/\r?\n/).find(Boolean)?.slice(0,60)||"无标题记录",data:{content:body,plainText:body,type,status:"inbox",tags:[],isPinned:false,linkedTaskIds:[]}});setActiveId(item.id);setMobileEditor(true);window.setTimeout(()=>titleRef.current?.focus(),30);return item;}
  async function quickSave(){const value=quick.trim();if(!value)return;await newNote("quick",value);setQuick("");notify("已保存到随手记");}
...
function QuickCapture({ workspace, close, notify, openNote }: { workspace: WorkspaceActions; close:()=>void; notify:(message:string)=>void; openNote:(item:WorkspaceItem)=>void }) {
  ...
  return <Modal title="快速记录" close={close}><p className="capture-intro">{new Date().toLocaleString("zh-CN",{month:"long",day:"numeric",hour:"2-digit",minute:"2-digit"})} · 草稿会自动保留</p><textarea className="capture-note-input" autoFocus value={draft} onChange={(event)=>setDraft(event.target.value)} onKeyDown={(event)=>{if((event.ctrlKey||event.metaKey)&&event.key==="Enter"){event.preventDefault();void save();}}} placeholder="记下此刻的想法、会议内容或待办事项……"/><div className="form-actions"><button className="secondary-button" type="button" onClick={close}>取消</button><button className="secondary-button" type="button" disabled={!draft.trim()} onClick={()=>void save(true)}>展开编辑</button><button className="primary-button" type="button" disabled={!draft.trim()} onClick={()=>void save()}><span>保存到随手记</span><span className="button-orb">✓</span></button></div></Modal>;
}
