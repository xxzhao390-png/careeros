...
function ThoughtsView({ items, createItem, updateItem, removeItem, uploadFile, notify, selectedItem, openItem }: WorkspaceActions & { notify:(message:string)=>void; selectedItem?: WorkspaceItem; openItem:(item:WorkspaceItem)=>void }) {
  ...
  async function createAiTasks(candidates:TaskCandidate[]){if(!active||!candidates.length)return 0;const existing=(active.data.taskLinks as Array<Record<string,string>>|undefined)||[];const fresh=candidates.filter((candidate)=>!existing.some((link)=>link.sourceText===candidate.evidenceText));const created:WorkspaceItem[]=[];for(const candidate of fresh){created.push(await createItem({kind:"task",title:candidate.title,data:{dueDate:candidate.dueAt,priority:candidate.priority,category:candidate.category,note:candidate.description,done:false,sourceNoteId:active.id,sourceText:candidate.evidenceText,aiGenerated:true,aiConfidence:candidate.confidence}}));}if(created.length){await patchNote({linkedTaskIds:[...new Set([...list(active,"linkedTaskIds"),...created.map((task)=>task.id)])],taskLinks:[...existing,...created.map((task,index)=>({taskId:task.id,sourceText:fresh[index].evidenceText,createdAt:new Date().toISOString()}))]});}return created.length;}
  async function organizeNote(){
    ...
    try{
      ...
      notify(taskCount?`AI 整理已应用，并自动创建 ${taskCount} 个关联任务`:"AI 整理结果已应用，原文保持不变");
