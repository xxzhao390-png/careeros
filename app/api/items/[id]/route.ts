...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const existing = await env.DB.prepare("SELECT * FROM workspace_items WHERE id = ?").bind(id).first();
  ...
  await env.DB.prepare("UPDATE workspace_items SET title = ?, data = ?, updated_at = ? WHERE id = ?")
    .bind(title, JSON.stringify(data), updatedAt, id).run();
  return Response.json({ item: { ...current, title, data, updatedAt } });
}

export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  const { id } = await context.params;
  const result = await env.DB.prepare("DELETE FROM workspace_items WHERE id = ?").bind(id).run();
  if (!result.meta.changes) return Response.json({ error: "记录不存在" }, { status: 404 });
