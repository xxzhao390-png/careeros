...
export async function POST(request: Request) {
  ...
  if (text.length < 20 || text.length > 30000) return Response.json({ error: "请粘贴 20～30000 字的原始知识内容", code: "VALIDATION_ERROR" }, { status: 400 });
  try { return Response.json({ result: await runAi(user, "knowledge_import", text) }); }
